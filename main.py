...
from loader import LQLDatasetTrain, LQLDatasetValid, collate_train, collate_test
...
from KGFM.models import KnowledgeEncoder
import time
import warnings
...
for name, param in model.named_parameters():
    if ('llm' in name) and ('structure' not in name):
...
lr_scheduler = get_constant_schedule_with_warmup(
    optimizer=optimzer,
    num_warmup_steps=int(len(data_loader_train) // 4), #50 * args.gradient_accumulation_steps, #int(num_training_steps * args.warmup_rate),
    # num_training_steps=num_training_steps
)
model, data_loader_train, data_loader_test, optimzer, lr_scheduler = accelerator.prepare(model, data_loader_train, data_loader_test, optimzer, lr_scheduler)

def evaluate(data_loader, model, graph, filtered_data):
...
#  total_mrr_per_device) = evaluate(data_loader_test, model, data_test.test_graph.to(args.device), filtered_data)
# total_hit1, total_hit3, total_hit10, total_mrr = [], [], [], []
...
# accelerator.print(
#     f"Mean Hit1 {np.mean(total_hit1)}\n"
#     f"Mean Hit3 {np.mean(total_hit3)}\n"
#     f"Mean Hit10 {np.mean(total_hit10)}\n"
#     f"Mean MRR {np.mean(total_mrr)}"
# )
# accelerator.log({"Mean Hit1": np.mean(total_hit1)}, step=-1)
# accelerator.log({"Mean Hit3": np.mean(total_hit3)}, step=-1)
# accelerator.log({"Mean Hit10": np.mean(total_hit10)}, step=-1)
# accelerator.log({"Mean MRR": np.mean(total_mrr)}, step=-1)
total_step = 0
for epoch in range(args.epochs):
    ...
    for batch in tqdm(data_loader_train):
        with accelerator.accumulate(model): # loss1, loss2, kl1, kl2
            loss1, loss2, kl1, kl2 = model(batch=batch, graph=data_train.train_graph.to(args.device),
                         entity2orgTokens=data_train.entity2orgTokens_train,
                         relation2orgTokens=data_train.relation2orgTokens_train,
                         tokenizer=tokenizer_train, output_hidden_states=True)
            loss = (1 - args.kd_lambda) * loss1 + args.kd_lambda * kl1 + (1 - args.kd_lambda) * loss2 + args.kd_lambda * kl2
            ...
            accelerator.log({"main_loss": loss.item()}, step=total_step)
            accelerator.log({"loss1": loss1.item()}, step=total_step)
            accelerator.log({"loss2": loss2.item()}, step=total_step)
            # accelerator.log({"loss_nbf": 0.5 * loss_nbf.item()}, step=total_step)
            accelerator.log({"kl1": kl1.item()}, step=total_step)
            accelerator.log({"kl2": kl2.item()}, step=total_step)
            ...
            step += 1
            total_step += 1
    train_ppl = total_loss / len(data_loader_train)
    ...
    accelerator.log({"Mean Hit1": np.mean(total_hit1)}, step=epoch)
    accelerator.log({"Mean Hit3": np.mean(total_hit3)}, step=epoch)
    accelerator.log({"Mean Hit10": np.mean(total_hit10)}, step=epoch)
    accelerator.log({"Mean MRR": np.mean(total_mrr)}, step=epoch)
