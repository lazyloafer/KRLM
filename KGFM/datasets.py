...
from motif.tasks import build_relation_graph, build_relation_hypergraph 


class GrailInductiveDataset(InMemoryDataset):

    def __init__(self, device, root, version, transform=None, 
                 pre_transform=T.Compose([build_relation_graph, build_relation_hypergraph]), 
    ...
    def process(self):
        ...
        train_data = Data(edge_index=train_fact_index, edge_type=train_fact_type, num_nodes=len(inv_train_entity_vocab),
                          target_edge_index=edge_index[:, train_slice], target_edge_type=edge_type[train_slice], num_relations=num_relations*2, device = self.device)
        valid_data = Data(edge_index=train_fact_index, edge_type=train_fact_type, num_nodes=len(inv_train_entity_vocab),
                          target_edge_index=edge_index[:, valid_slice], target_edge_type=edge_type[valid_slice], num_relations=num_relations*2, device = self.device)
        test_data = Data(edge_index=test_fact_index, edge_type=test_fact_type, num_nodes=len(inv_test_entity_vocab),
                         target_edge_index=edge_index[:, test_slice], target_edge_type=edge_type[test_slice], num_relations=num_relations*2, device = self.device)
...
class FB15k237Inductive(GrailInductiveDataset):
    ...
    def __init__(self, root, version, device):
        super().__init__(root=root, version=version, device = device)
...
class WN18RRInductive(GrailInductiveDataset):
    ...
    def __init__(self, root, version, device):
        super().__init__(root = root, device = device, version = version)
...
def FB15k237(root,device):
    ...
    train_data = Data(edge_index=data.edge_index, edge_type=data.edge_type, num_nodes=data.num_nodes,
                        ...
                        num_relations=dataset.num_relations, device = device)
    ...
    # build relation hypergraphs
    train_data = build_relation_hypergraph(train_data)
    valid_data = build_relation_hypergraph(valid_data)
    test_data = build_relation_hypergraph(test_data)

    dataset.data, dataset.slices = dataset.collate([train_data, valid_data, test_data])
    return dataset

def WN18RR(root, device):
...
class TransductiveDataset(InMemoryDataset):
    ...
    def __init__(self, root, device, transform=None, pre_transform=T.Compose([build_relation_graph, build_relation_hypergraph]), **kwargs):
        self.device = device
        super().__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    def process(self):
        ...
        train_data = Data(edge_index=train_edges, edge_type=train_etypes, num_nodes=num_node,
                          target_edge_index=train_target_edges, target_edge_type=train_target_etypes, num_relations=num_relations*2, device = self.device)
        valid_data = Data(edge_index=train_edges, edge_type=train_etypes, num_nodes=num_node,
                          target_edge_index=valid_edges, target_edge_type=valid_etypes, num_relations=num_relations*2, device = self.device)
        test_data = Data(edge_index=train_edges, edge_type=train_etypes, num_nodes=num_node,
                         target_edge_index=test_edges, target_edge_type=test_etypes, num_relations=num_relations*2, device = self.device)
...
class CoDExSmall(CoDEx):
    ...
    def __init__(self, device, root):
        super(CoDExSmall, self).__init__(root=root, device=device, size='s')
...
class CoDExMedium(CoDEx):
    ...
    name = "codex-m"
    def __init__(self, device, root):
        super(CoDExMedium, self).__init__(root=root, device = device, size='m')
...
class CoDExLarge(CoDEx):
    ...
    name = "codex-l"
    def __init__(self, device,  root):
        super(CoDExLarge, self).__init__(root=root, device = device, size='l')
...
class InductiveDataset(InMemoryDataset):
    ...
    def __init__(self, root, version, device, transform=None, pre_transform=T.Compose([build_relation_graph, build_relation_hypergraph]),  **kwargs):
    ...
    def process(self):
        ...
        train_data = Data(edge_index=train_fact_index, edge_type=train_fact_type, num_nodes=num_train_nodes,
                          target_edge_index=train_target_edges, target_edge_type=train_target_etypes, num_relations=num_train_rels*2, device = self.device)
        valid_data = Data(edge_index=inf_edges if self.valid_on_inf else train_fact_index, 
                          ...
                          num_relations=inference_num_rels*2 if self.valid_on_inf else num_train_rels*2, device = self.device)
        test_data = Data(edge_index=inf_edges, edge_type=inf_etypes, num_nodes=inference_num_nodes,
                         target_edge_index=inf_test_edges[:, :2].T, target_edge_type=inf_test_edges[:, 2], num_relations=inference_num_rels*2, device = self.device)
...
class ILPC2022SmallInductive(ILPC2022):
    
    def __init__(self, root, device, **kwargs):
        super(ILPC2022SmallInductive, self).__init__(root=root, device=device, version="small")
    
class ILPC2022LargeInductive(ILPC2022):
        
    def __init__(self, root, device, **kwargs):
        super(ILPC2022LargeInductive, self).__init__(root=root, device=device, version="large")
...
class HM(InductiveDataset):
    ...
    def __init__(self, root, version, device, **kwargs):
        version = self.versions[version]
        self.device = device
        super().__init__(root=root, version=version, device = device, **kwargs)
...
class JointDataset(InMemoryDataset):
    ...
    datasets_map = {
        ...
        'WN18RRInductive': WN18RRInductive,
        ...
        'WKIngram': WKIngram,
        'NLIngram': NLIngram,
        'WikiTopicsMT1': WikiTopicsMT1,
        'WikiTopicsMT2': WikiTopicsMT2,
        'WikiTopicsMT3': WikiTopicsMT3,
        'WikiTopicsMT4': WikiTopicsMT4,
        'Metafam': Metafam,
        'FBNELL': FBNELL,
    }

    def __init__(self, root, graphs, device, transform=None, pre_transform=None):


        self.graphs = [self.datasets_map[ds.split(":")[0]](root=root, version=(ds.split(":")[1]), device = device) if ":" in ds else self.datasets_map[ds](root=root, device = device) for ds in graphs]
