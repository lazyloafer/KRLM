...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.structure_fq = 3
        NBFdim = 64
        self.head_NBFdim = NBFdim // config.num_attention_heads
        if self.layer_idx % self.structure_fq == 0:
            self.structure_q_proj = nn.Linear(config.hidden_size, config.num_attention_heads * self.head_NBFdim,
                                              bias=False)
            # self.structure_k_proj = nn.Linear(NBFdim, self.num_key_value_heads * self.head_NBFdim, bias=config.attention_bias)
            self.structure_v_proj = nn.Linear(NBFdim, config.num_attention_heads * self.head_dim, bias=False)
    ...
    def eager_attention_forward(self,
        ...
        if self.layer_idx % self.structure_fq == 0:
            ...
            structure_query_states = structure_query_states.view(bsz, q_len, self.config.num_attention_heads,
                                                                 self.head_NBFdim).transpose(1,
                                                                                             2)
            structure_key_states = structure_key_states.view(bsz, structure_key_states.size(1),
                                                             self.config.num_attention_heads,
                                                             self.head_NBFdim).transpose(1, 2)
            structure_value_states = structure_value_states.view(bsz, structure_value_states.size(1),
                                                                 self.config.num_attention_heads,
                                                                 self.head_dim).transpose(1,
                                                                                          2)
            structure_attn_weights = torch.matmul(structure_query_states,
                                                  structure_key_states.transpose(2, 3)) / math.sqrt(
                self.head_dim)
